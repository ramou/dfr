...
std::cout << std::endl;
	...
	const auto passOverInputDealExactCounting =
		[&bucketCounts]
		(ELEM* start, ELEM *end, const uint8_t &currentByte, const uint8_t &countByte,const auto &targetBuckets) {
		if(start == end) return;
		const ELEM* element = start;
		const uint8_t *target = reinterpret_cast<const uint8_t*>(element) + currentByte;
		const uint8_t *countTarget = reinterpret_cast<const uint8_t*>(element) + countByte;
	...
	const auto passOverInputCounting =
        ...
        const auto passOverInputCountingAndGatherLiveBits =
                [&bucketCounts, &livebits, &bitmask]
                (ELEM* start, ELEM *end, const uint8_t &countByte) {
                if(start == end) return;
                const ELEM* element = start;
                const uint8_t *countTarget = reinterpret_cast<const uint8_t*>(element) + countByte;
                ...
                for(unsigned i = 0; i < len; ++i, livebits |= bitmask ^ reinterpret_cast<INT>(*(element++)), bucketCounts[*countTarget]++, countTarget+=sizeof(ELEM));
        };
	...
	//Should we assign overflow's starting position here? Is the shrinking of source the cause of this pain or is it always pointing to a suitable buffer?
			...
			passOverInputCounting(source, source+length, countedByte);
...
std::cout << "After 1 pass Top Bytes: " << topBytesSize << " Bottom Bytes: " << bottomBytesSize << std::endl;
        ...
        std::chrono::high_resolution_clock::time_point start = std::chrono::high_resolution_clock::now();
			...
			if(hasByteLists) {
				...
				passOverInputCountingAndGatherLiveBits(source, source+length, countedByte);
				buildByteLists(currentByte);
				#ifdef DEBUG
...
std::cout << "Initial Counting Pass Time: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << "\n";
start = std::chrono::high_resolution_clock::now();

#endif
#endif
			if(setByteListHere && currentByte != bytes[0]) return true;

			for(int i = 0; i < (numBytes - 1); i++) {
				passOverInputDealExactCounting(source, source+length, bytes[i], bytes[i+1], destinationBuckets);
				swap();
				convertCountsToContiguousBuckets(bucketCounts, destinationBuckets, destination);
				#ifdef DEBUG
				#ifdef TIMINGS
